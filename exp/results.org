...
#+begin_src shell
  ...
  merge_commits(){
    local prog=$1;
    grep $prog results/cc-merges.txt|wc -l
  }

  merge_line_conflicts(){
    local prog=$1;
    grep $prog results/cc-merges.txt|grep -Pc "\tconflict"
  }

  merge_diff3_commits(){
    local prog=$1;
    grep $prog results/js-stats.txt|awk 'BEGIN{count=0}{if($2 != 0){count++}}END{print count}'
  }

  for prog in $(cat results/cc-merges.txt|awk '{print $1}'|sort|uniq);do
    echo -ne "$prog"
